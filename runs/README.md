...
| `imo2026_p1-opt-rlcr-deepseek` | same, A/B rerun on the post-handoff-#5 optimized gym | **solved** (complete, build_strict, 11.4 s) | 0 | 7 391 | 117.3 | $1.90 (mostly off-peak) $3.81 if at peak | same humanfia refs as above | 0 | 3 | **Optimization confirmed: 2.0× wall, 2.1× agent-active, 0 MCP timeouts (was 12), diagnostics 47→18 calls at 0.3 s median (incremental sync), arbiter 44→11 s.** 0 metis/smt/blast; builder went scratch-first (42 `run_code`, 0 err). Confound: rule 6 changed between runs. See ANALYSIS.md |
